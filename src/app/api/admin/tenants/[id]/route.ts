...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const formattedTenant = {
      ...
      favorites: tenant.favorites.map((favorite: { id: number; property: { id: number; name: string; address: string; manager: { name: string }}}) => ({
        id: favorite.id,
      ...
      applications: tenant.applications.map((application: { id: number; property: { id: number; name: string }; status: string; createdAt: Date }) => ({
        id: application.id,
      ...
      leases: tenant.leases.map((lease: { id: number; property: { id: number; name: string }; startDate: Date; endDate: Date; rent: number }) => ({
        id: lease.id,
